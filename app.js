...
app.get("/branches", function(req, res){
    pool.query('select филиалы.id, филиалы.Адрес, филиалы.Телефон, сотрудники.Фамилия from `филиалы` join сотрудники on `филиалы`.id_администратор = сотрудники.id', function(err, data) {
      if(err) return console.log(err);
...
app.get("/employers", function(req, res){
    pool.query('select сотрудники.id, Фамилия, Имя, Телефон, Оклад, Адрес, Название from `сотрудники` join должности on `сотрудники`.id_должность = должности.id', function(err, data) {
      if(err) return console.log(err);
...
app.get("/jobtitle", function(req, res){
    pool.query('select id, Название, `График работы` as график from должности', function(err, data) {
      if(err) return console.log(err);
...
app.get("/edit/:id", function(req, res){
  ...
  pool.query("SELECT * FROM `учет оказания услуг` WHERE id=?", [id], function(err, data) {
    if(err) return console.log(err);
     res.render("edit.hbs", {
        user: data[0]
    });
...
app.post("/edit", urlencodedParser, function (req, res) {
  ...
  pool.query("UPDATE `Учет оказания услуг` SET Дата=?, id_услуга=?, id_сотрудник=?, id_клиент=?, id_филиал=? WHERE id=?", [date, service, employer, customer, branch, id], function(err, data) {
    if(err) return console.log(err);
    res.redirect("/");
  });
});
...
app.post("/delete/:id", function(req, res){
  ...
  const id = req.params.id;
  pool.query("DELETE FROM `учет оказания услуг` WHERE id=?", [id], function(err, data) {
    if(err) return console.log(err);
    res.redirect("/");
  });
});
